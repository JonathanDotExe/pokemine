...
public class DamagePercentProcedure extends SingleUserProcedure {
//
	private float percent = 1;
	private DamageClass defenseDamageClass;

	public DamagePercentProcedure(boolean performer, float percent, DamageClass defenseDamageClass) {
		super (performer);
...
//		performer.setHealth(defender.getHealth() - Math.round(damage * recoil));
		...
		defender.damagePercent(performer, percent, defenseDamageClass == null ? instance.getMove().getDamageClass() : defenseDamageClass);
//		performer.setHealth(performer.getHealth() - Math.round(damage * recoil));
	...
	public static DamagePercentProcedure deserialize (Map<String, Object> map) {
		TypedMap tMap = new TypedMap(map);
		DamagePercentProcedure proc =  new DamagePercentProcedure(false, tMap.getFloat("percent"), map.containsKey("defenseDamageClass") ? tMap.getEnum("defenseDamageClass", DamageClass.class, null) : null);
		proc.load(map);
		return proc;
	}
	...
	public String toString() {
		return "DamagePercentProcedure [percent=" + percent + ", defenseDamageClass=" + defenseDamageClass + "]";
	}
